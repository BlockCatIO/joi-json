...
describe("lib/index", function() {
  ...
  describe(".builder", function() {
    ...
    it("fail: when engines do not exist", function() {
      ...
      expect(index.builder.bind(null)).to.throw(
        "cannot find validation engine",
      );
    });
  });
  ...
  describe("Joi schema cases", function() {
    ...
    it("object", function() {
      ...
      expect(schema._inner.children[0].schema._flags.presence).to.equal(
        "required",
      );
    ...
    it("alternatives []", function() {
      ...
      expect(schema._inner.matches[0].schema._flags.presence).to.equal(
        "required",
      );

      expect(schema._inner.matches[1].schema._type).to.equal("number");
      expect(schema._inner.matches[1].schema._flags.presence).to.equal(
        "required",
      );

      let result1 = joi.validate(
        "3952469a-1b45-4d5e-825c-e25ec8b57cf6",
        schema,
      );
